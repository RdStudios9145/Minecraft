...
class Block {
	pos = null as three.Vector3;
	blockType = null as BlockType;

	geometry = null as three.BoxGeometry;
	mat = null as three.MeshStandardMaterial;
	cube = null as three.Mesh;

	// rigidbody = null as cannon.Body;

	constructor(_pos: three.Vector3, _type: BlockType = BlockTypes.Grass) {
		this.pos = _pos;
		this.blockType = _type;

		this.geometry = new three.BoxGeometry();

		// let toHex = (val: number) => {
		...
		// 	return s[Math.abs(val) % s.length];
		// }

		// let s = 8;
		const texture = new three.TextureLoader().load(`/img/${_type.texture[0]}_carried.png`);
		texture.minFilter = three.NearestFilter;
		texture.magFilter = three.NearestFilter;
		this.mat = new three.MeshStandardMaterial({ color: _type.color, flatShading: true, roughness: 1, map: texture });
